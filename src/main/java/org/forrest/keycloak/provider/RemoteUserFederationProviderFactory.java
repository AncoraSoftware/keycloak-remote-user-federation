...
public class RemoteUserFederationProviderFactory implements UserStorageProviderFactory<RemoteUserFederationProvider> {
    ...
    public RemoteUserFederationProviderFactory() {
        ...
        configMetadata = ProviderConfigurationBuilder.create()
            ...
            .property()
                ...
                .label("Define endpoint for verify password")
                .type(ProviderConfigProperty.STRING_TYPE)
                .defaultValue("/verify")
                .add()
            .property()
                .name(SEARCH_USER_ENDPOINT)
                .label("Define endpoint for search users")
                .type(ProviderConfigProperty.STRING_TYPE)
                .defaultValue("/search")
                .add()
            .property()
                .name(COUNT_USER_ENDPOINT)
                .label("Define endpoint for count users")
                .type(ProviderConfigProperty.STRING_TYPE)
                .defaultValue("/count")
                .add()
            .property()
                .name(ADD_ROLES_TO_TOKEN)
                .label("Add roles to token")
                .type(ProviderConfigProperty.BOOLEAN_TYPE)
                .defaultValue(true)
                .add()
            .property()
                .name(RESOURCE_CLIENT_ID)
                .label("Resource Client ID")
                .helpText("Client ID to use for scoping user roles. If not specified or client doesn't exist, roles will be added as realm roles.")
                .type(ProviderConfigProperty.STRING_TYPE)
                .add()
            .property()
                .name(HEADERS_TO_FORWARD)
                .label("Headers to forward")
                .helpText("List of headers to forward to the remote server.")
                .type(ProviderConfigProperty.MULTIVALUED_STRING_TYPE)
                .add()
            .property()
                .name(DEBUG_ENABLED)
                .label("Enable detail logs")
                .type(ProviderConfigProperty.BOOLEAN_TYPE)
                .defaultValue("false")
                .add()
            .build();
    ...
    public RemoteUserFederationProvider create(KeycloakSession session, ComponentModel model) {
        ...
        return new RemoteUserFederationProvider(
            session,
            model, 
            new UserService(session, model, debugLoggingEnabled), 
            debugLoggingEnabled);
    }
